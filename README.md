...
Open **Review** to select Numbers, Food & drink, Greetings & politeness, Everyday conversation, Transportation & directions, Places & countries, Time, Basic verbs, or Adjectives. Each category lists its active vocabulary in German and English, with a **Pronunciation help** column. The local `pronunciationHelp` field contains approximate English-friendly respellings, including stress and syllable breaks. Expand **How to read pronunciation help** for the sound key, especially German ch and ü; the hints supplement rather than replace German audio. The table scrolls horizontally on small screens. For additional listening practice, see the [Goethe-Institut pronunciation trainer](https://www.goethe.de/en/spr/ueb/ast.html). Retired vocabulary is excluded.
