...
import { GermanVoicePicker } from './components/GermanVoicePicker.jsx';
...
function Quiz({ userId, direction, onDirectionChange, settingsError, onSaved, speech }) {
  ...
  function start() { setDiscarded(false); setQuiz({ direction, id: crypto.randomUUID(), startedAt: new Date().toISOString(), questions: selectQuestions(vocabulary, questionCount) }); setAnswers([]); setInput(''); setGraded(null); setFinished(null); setSaveState(''); setError(''); }
  function endQuiz() {
  ...
  function next() {
    ...
    if (nextAnswers.length === quiz.questions.length) {
      ...
      const result = { session: { id: quiz.id, user_id: userId, direction: quiz.direction, started_at: quiz.startedAt, completed_at: completedAt, correct_count: score.correct, total_count: quiz.questions.length, duration_seconds: Math.max(0, Math.round((Date.parse(completedAt) - Date.parse(quiz.startedAt)) / 1000)) }, answers: nextAnswers.map(({ autoCorrect, ...answer }) => answer) };
      setFinished(result); void persist(result);
    }
  }
  if (!quiz) return <>{discarded && <Notice>Quiz ended. No results were saved.</Notice>}<div className="page-heading"><span className="eyebrow">DAILY PRACTICE</span><h1>Small steps. Real progress.</h1><p>Take a moment to make a little more German your own.</p></div><section className="card quiz-intro"><div className="badge" lang="de">Los geht’s!</div><h2>Everyday German</h2><p>Greetings, café orders, getting around, and the words in between.</p><div className="pills"><span>{questionCount} {questionCount === 1 ? 'question' : 'questions'}</span><span>{quizText({}, direction).label}</span><span>At your pace</span></div><div className="settings"><label htmlFor="quiz-direction">Quiz direction</label><select id="quiz-direction" value={direction} onChange={event => onDirectionChange(event.target.value)} aria-describedby="direction-help"><option value="de-en">Show German → guess English</option><option value="en-de">Show English → guess German</option></select><p id="direction-help" className="muted">Applies to the next quiz you start. Remembered for your account on this browser.</p>{settingsError && <Notice error>{settingsError}</Notice>}<div className="quiz-length"><label htmlFor="quiz-length">Number of questions</label><select id="quiz-length" value={questionCount} onChange={event => setQuestionCount(Number(event.target.value))}>{[3, 10, 20, 30].map(count => <option key={count} value={count}>{count} questions</option>)}<option value={vocabulary.length}>All ({vocabulary.length} questions)</option></select></div></div><button className="primary" onClick={start}>Start a quiz →</button><p className="muted">See the answer, check your understanding, and adjust the grade if needed.</p></section><div className="tip"><strong>A little tip</strong><p>{direction === 'en-de' ? 'For German nouns, include the article (der, die, das). Use the infinitive for verbs, as in “essen”.' : 'For English nouns, translate the word without “the”. For verbs, use “to”, as in “to eat”.'} Natural alternatives can be marked correct after submitting.</p></div></>;
  if (finished) {
    const score = scoreQuiz(answers);
    return <><div className="page-heading"><span className="eyebrow">PRACTICE COMPLETE</span><h1>One step further.</h1><p>Every answer is a chance to learn.</p></div><section className="card"><div className="result-score">{score.correct}<span> / {score.total}</span></div><h2>{score.percentage}% correct</h2><p>{score.correct} correct · {score.incorrect} incorrect · {formatDuration(finished.session.duration_seconds)}</p>{saveState === 'saving' && <Notice>Saving your quiz…</Notice>}{saveState === 'saved' && <Notice>Your quiz is saved. Find it in History.</Notice>}{error && <Notice error>{error}</Notice>}{saveState === 'failed' && <button onClick={() => persist(finished)}>Retry saving</button>}<h3>Words to revisit</h3>{!score.incorrect ? <p>You got every word right. Schön gemacht!</p> : <ul className="missed">{answers.filter(a => !a.correct).map(a => <li key={a.id}><strong lang="de">{vocabularyById[a.vocabulary_id].german}</strong><span>{vocabularyById[a.vocabulary_id].english}</span></li>)}</ul>}<button className="primary" disabled={saveState !== 'saved'} onClick={() => setQuiz(null)}>Start another quiz →</button></section></>;
  }
  const item = quiz.questions[answers.length];
  const text = quizText(item, quiz.direction);
  return <><div className="page-heading"><span className="eyebrow">EVERYDAY GERMAN</span><h1>A word at a time.</h1></div><section className="card question-card"><div className="question-meta"><span>Question {answers.length + 1} of {quiz.questions.length}</span><span>{text.label}</span></div><progress value={answers.length} max={quiz.questions.length} aria-label="Quiz progress"/><p className="muted">How would you say this in {text.answerLanguage}?</p><h2 className="german" lang={text.promptLang}>{text.prompt}</h2><form onSubmit={event => { event.preventDefault(); if (!input.trim() || graded) return; const correct = compareAnswer(input, text.expected); setGraded({ id: crypto.randomUUID(), session_id: quiz.id, user_id: userId, vocabulary_id: item.id, user_answer: input.trim(), correct, autoCorrect: correct, answered_at: new Date().toISOString() }); speak(item.german); }}><label htmlFor="answer">Your {text.answerLanguage} translation</label><input ref={inputRef} id="answer" lang={text.answerLang} value={input} onChange={event => setInput(event.target.value)} maxLength={500} disabled={!!graded} autoComplete="off" placeholder="Type your answer…"/>{!graded && <button className="primary" disabled={!input.trim()}>Submit answer →</button>}</form>{graded && <div className="feedback" aria-live="polite"><span className="eyebrow">{graded.autoCorrect ? 'AUTOMATIC MATCH' : 'NO AUTOMATIC MATCH'}</span><h3 lang={text.answerLang}>{text.expected}</h3><GermanVoicePicker speech={speech} id="quiz-voice"/><div className="button-row"><button type="button" disabled={!audioSupported} onClick={() => speak(item.german)}>Replay German audio</button></div>{!audioSupported && <p className="muted" role="status">Audio is not supported in this browser. Try a browser with speech synthesis support.</p>}{audioError && <Notice error>{audioError}</Notice>}{item.exampleGerman && <section className="quiz-example" aria-label="Example sentence"><h3>Example sentence</h3><p lang="de">{item.exampleGerman}</p><p lang="en" className="muted">{item.exampleEnglish}</p><button type="button" disabled={!audioSupported} onClick={() => speak(item.exampleGerman)}>Play German sentence</button></section>}<p>Current grade: <strong>{graded.correct ? 'Correct' : 'Incorrect'}</strong></p><div className="button-row"><button aria-pressed={graded.correct} onClick={() => setGraded({ ...graded, correct: true })}>Mark correct</button><button aria-pressed={!graded.correct} onClick={() => setGraded({ ...graded, correct: false })}>Mark incorrect</button></div><button className="primary" onClick={next}>{answers.length === quiz.questions.length - 1 ? 'Finish quiz →' : 'Next question →'}</button></div>}<div className="quiz-exit"><button type="button" onClick={endQuiz}>End quiz</button><small>Ends this quiz without saving any results.</small></div></section></>;
}
...
function Workspace({ session }) {
  ...
  const getPage = () => ['quiz', 'review', 'history', 'progress', 'account'].includes(location.hash.slice(1)) ? location.hash.slice(1) : 'quiz';
  ...
  return <><header><a className="brand" href="#quiz"><span className="brand-icon">w.</span>wortreise<span className="brand-caption">GERMAN, LITTLE BY LITTLE</span></a><nav aria-label="Main navigation">{['quiz', 'review', 'history', 'progress', 'account'].map(name => <a key={name} href={`#${name}`} aria-current={page === name ? 'page' : undefined}>{name[0].toUpperCase() + name.slice(1)}</a>)}</nav></header><main>{/* Keep an active quiz mounted when visiting other tabs. */}<div hidden={page !== 'quiz'}><Quiz speech={speech} userId={session.user.id} direction={direction} onDirectionChange={changeDirection} settingsError={settingsError} onSaved={() => setRevision(r => r + 1)}/></div>{(page === 'history' || page === 'progress') && (loading ? <Notice>Loading your practice history…</Notice> : error ? <><Notice error>{error}</Notice><button onClick={() => setRevision(r => r + 1)}>Try again</button></> : page === 'history' ? <History {...history}/> : <Progress {...history}/>)}{page === 'review' && <Review speech={speech}/>} {page === 'account' && <section className="card account"><span className="eyebrow">YOUR ACCOUNT</span><h1>A place for your progress.</h1><p>Logged in as <strong>{session.user.email}</strong></p><p className="muted">Finished quizzes sync to your account. Finish and save your current quiz before logging out.</p>{error && <Notice error>{error}</Notice>}<button disabled={loggingOut} onClick={logout}>{loggingOut ? 'Logging out…' : 'Log out'}</button></section>}</main><footer>A few words today. A little more confidence tomorrow.</footer></>;
}
function App() {
