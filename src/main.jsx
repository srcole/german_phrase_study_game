...
import { calculateProgress, compareAnswer, formatDuration, scoreQuiz, selectQuestions } from './lib/quiz';
...
function Quiz({ userId, onSaved }) {
  ...
  function start() { setQuiz({ id: crypto.randomUUID(), startedAt: new Date().toISOString(), questions: selectQuestions(vocabulary) }); setAnswers([]); setInput(''); setGraded(null); setFinished(null); setSaveState(''); setError(''); }
  ...
  function next() {
    ...
    if (nextAnswers.length === 10) {
      ...
      const result = { session: { id: quiz.id, user_id: userId, started_at: quiz.startedAt, completed_at: completedAt, correct_count: score.correct, total_count: 10, duration_seconds: Math.max(0, Math.round((Date.parse(completedAt) - Date.parse(quiz.startedAt)) / 1000)) }, answers: nextAnswers.map(({ autoCorrect, ...answer }) => answer) };
      setFinished(result); void persist(result);
    }
  }
  if (!quiz) return <><div className="page-heading"><span className="eyebrow">DAILY PRACTICE</span><h1>Small steps. Real progress.</h1><p>Take a moment to make a little more German your own.</p></div><section className="card quiz-intro"><div className="badge" lang="de">Los geht’s!</div><h2>Everyday German</h2><p>Greetings, café orders, getting around, and the words in between.</p><div className="pills"><span>10 questions</span><span>Type in English</span><span>At your pace</span></div><button className="primary" onClick={start}>Start a quiz →</button><p className="muted">See the answer, check your understanding, and adjust the grade if needed.</p></section><div className="tip"><strong>A little tip</strong><p>For nouns, translate the word without “the”. For verbs, use “to”, as in “to eat”. Natural alternatives can be marked correct after submitting.</p></div></>;
  if (finished) {
  ...
  return <><div className="page-heading"><span className="eyebrow">EVERYDAY GERMAN</span><h1>A word at a time.</h1></div><section className="card question-card"><div className="question-meta"><span>Question {answers.length + 1} of 10</span><span>German → English</span></div><progress value={answers.length} max="10" aria-label="Quiz progress"/><p className="muted">How would you say this in English?</p><h2 className="german" lang="de">{item.german}</h2><form onSubmit={event => { event.preventDefault(); if (!input.trim() || graded) return; const correct = compareAnswer(input, item.english); setGraded({ id: crypto.randomUUID(), session_id: quiz.id, user_id: userId, vocabulary_id: item.id, user_answer: input.trim(), correct, autoCorrect: correct, answered_at: new Date().toISOString() }); }}><label htmlFor="answer">Your English translation</label><input ref={inputRef} id="answer" value={input} onChange={event => setInput(event.target.value)} maxLength={500} disabled={!!graded} autoComplete="off" placeholder="Type your answer…"/>{!graded && <button className="primary" disabled={!input.trim()}>Submit answer →</button>}</form>{graded && <div className="feedback" aria-live="polite"><span className="eyebrow">{graded.autoCorrect ? 'AUTOMATIC MATCH' : 'NO AUTOMATIC MATCH'}</span><h3>{item.english}</h3>{item.exampleGerman && <p><span lang="de">{item.exampleGerman}</span><br/><span className="muted">{item.exampleEnglish}</span></p>}<p>Current grade: <strong>{graded.correct ? 'Correct' : 'Incorrect'}</strong></p><div className="button-row"><button aria-pressed={graded.correct} onClick={() => setGraded({ ...graded, correct: true })}>Mark correct</button><button aria-pressed={!graded.correct} onClick={() => setGraded({ ...graded, correct: false })}>Mark incorrect</button></div><button className="primary" onClick={next}>{answers.length === 9 ? 'Finish quiz →' : 'Next question →'}</button></div>}</section></>;
}
function History({ sessions, answers }) {
  return <><div className="page-heading"><span className="eyebrow">YOUR PRACTICE JOURNAL</span><h1>Look how far you’ve come.</h1><p>Every completed quiz, all in one place.</p></div>{!sessions.length ? <Empty/> : sessions.map(session => {
    ...
    return <details className="card history-item" key={session.id}><summary><span>{new Date(session.completed_at).toLocaleString()}</span><strong>{session.correct_count}/{session.total_count} · {Math.round(session.correct_count / session.total_count * 100)}%</strong><span>{formatDuration(session.duration_seconds)}</span></summary>{rows.length !== session.total_count && <Notice error>Answer details are incomplete. If the original quiz is still open, use Retry saving there.</Notice>}<div className="table-scroll"><table><thead><tr><th>German</th><th>Your answer</th><th>Expected</th><th>Result</th></tr></thead><tbody>{rows.map(a => <tr key={a.id}><td lang="de">{vocabularyById[a.vocabulary_id]?.german || a.vocabulary_id}</td><td>{a.user_answer}</td><td>{vocabularyById[a.vocabulary_id]?.english || 'Vocabulary unavailable'}</td><td className={a.correct ? 'correct' : 'incorrect'}>{a.correct ? 'Correct' : 'Incorrect'}</td></tr>)}</tbody></table></div></details>;
  })}</>;
}
